...
RSpec.describe Signature, type: :model do
  ...
  describe "read-only attributes" do
    [
      ...
      describe "##{attribute}" do
        let(:signature) { FactoryGirl.create(:signature, attribute => value) }
  ...
  describe ".missing_constituency_id" do
    let!(:signature_1) { FactoryGirl.create(:validated_signature, validated_at: 2.weeks.ago) }
    let!(:signature_2) { FactoryGirl.create(:validated_signature, constituency_id: "3415") }
    let!(:signature_3) { FactoryGirl.create(:validated_signature, location_code: "US") }
    let!(:signature_4) { FactoryGirl.create(:validated_signature) }
  ...
  describe '#validate!' do
    let(:signature) { FactoryGirl.create(:pending_signature, attributes) }
    let(:location_code) { "GB" }
    let(:postcode) { "SW1A 1AA" }
  ...
  describe "#constituency" do
    let(:signature) { FactoryGirl.build(:signature, attributes) }
